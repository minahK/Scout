...
<style>
:root { --color-light:#ff9752; --color-base:#ee853f; --color-dark:#eb5e00; --color-border:#ddd; --color-bg:#f5f8fa; --color-font:#222; --color-white:#fff; }
...
.main{background:var(--color-white);border-right:1px solid var(--color-border)}
...
.sidebar a{display:block;padding:10px 14px;border-radius:999px;text-decoration:none;color:var(--color-font);font-weight:bold}
...
.cover{width:100%;height:200px;background:#cfd8dc url('https://images.unsplash.com/photo-1500530855697-b586d89ba3ee?q=80&w=1200&auto=format&fit=crop') center/cover no-repeat;position:relative}
.profile-head{padding:16px 20px 0 20px;position:relative}
.avatar-initial-lg{position:absolute;top:-60px;left:20px;width:120px;height:120px;border-radius:50%;border:4px solid var(--color-white);background:#eee;color:#444;display:flex;align-items:center;justify-content:center;font-size:48px;font-weight:800}
...
.btn-ghost:hover{background:#fff2e8}
.bio{margin-left:150px;padding-bottom:12px;border-bottom:1px solid var(--color-border)}
.bio h2{margin:0 0 4px 0}
.handle{color:#657786;font-size:14px}
.bio .counts{display:flex;gap:16px;margin-top:8px}
.bio .counts a{color:var(--color-font);text-decoration:none}
.bio .counts a strong{margin-right:4px}
.tabbar{position:sticky;top:0;z-index:10;background:var(--color-white);border-bottom:1px solid var(--color-border);display:flex;gap:0;padding:0 20px;font-weight:700}
.tabbar a{flex:1 1 0;text-align:center;padding:12px 0;border-bottom:3px solid transparent;color:var(--color-font);text-decoration:none;position:relative}
.tabbar a.active{color:var(--color-dark)}
.tabbar a.active::after{content:"";position:absolute;left:0;right:0;bottom:-1px;height:3px;background:var(--color-dark);border-radius:3px 3px 0 0}
/* 피드 */
.feed{max-width:680px;margin:0 auto}
.post{display:flex;gap:10px;padding:14px 20px;border-bottom:1px solid var(--color-border)}
.avatar-initial-sm{width:42px;height:42px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:#eee;color:#444;border:1px solid var(--color-border);font-weight:700}
.post .body{flex:1}
.post .meta{color:#657786;font-size:12px;margin-bottom:6px}
.post .content{font-size:14px;line-height:1.5;color:var(--color-font);white-space:pre-wrap}
.post .photo{margin-top:10px;border:1px solid #e6ecf0;border-radius:12px;max-height:420px;overflow:hidden}
.post .photo img{width:100%;display:block}
.actions{display:flex;gap:16px;margin-top:10px;color:#657786;font-size:13px}
.actions span{cursor:pointer}
.pinned{font-size:12px;color:#657786;margin:6px 0}
@media (max-width:1024px){ .layout{grid-template-columns:1fr} .sidebar,.rightbar{display:none} }
</style>
</head>
<body>
<div class="layout">
						...
						<div>
							...
							<c:choose>
								<c:when test="${not empty loginUser.handle}">
									<span style="font-size:12px;color:gray;">@<c:out value="${loginUser.handle}" /></span>
								</c:when>
								<c:otherwise>
									<span style="font-size:12px;color:gray;">@<c:out value="${loginUser.userId}" /></span>
								</c:otherwise>
							</c:choose>
						</div>
					</c:when>
						...
						<div>
							...
							<c:choose>
								<c:when test="${not empty user.handle}">
									<span style="font-size:12px;color:gray;">@<c:out value="${user.handle}" /></span>
								</c:when>
								<c:otherwise>
									<span style="font-size:12px;color:gray;">@<c:out value="${user.userId}" /></span>
								</c:otherwise>
							</c:choose>
						</div>
					</c:when>
	...
	<main class="main">
		...
		<c:choose>
			<c:when test="${not empty user}">
				<c:set var="displayName" value="${not empty user.nickname ? user.nickname : (not empty user.userId ? user.userId : 'Guest')}" />
			</c:when>
			<c:otherwise>
		...
		<div class="profile-head">
			...
			<div class="head-actions">
				<c:choose>
					...
					<c:otherwise>
						<form method="get" action="${ctx}/community/profile/edit" style="text-align:right;margin-bottom:12px;">
		...
		<div class="bio">
			...
			<div class="counts">
				<a href="${ctx}/community/profile/following?userId=${not empty user ? user.userId : 0}">
					<strong>${empty followingCount ? 0 : followingCount}</strong> 팔로잉
				</a>
				<a href="${ctx}/community/profile/followers?userId=${not empty user ? user.userId : 0}">
		...
		<nav class="tabbar">
			...
			<a href="${ctx}/community/profile/replies?userId=${not empty user ? user.userId : 0}">답글</a>
			...
			<c:if test="${not empty pinnedPost}">
				<div class="post">
					...
					<div class="body">
						...
						<div class="content"><c:out value="${pinnedPost.content}"/></div>
						<c:if test="${not empty pinnedPost.imageUrl}">
							<div class="photo"><img src="${ctx}/upload/${pinnedPost.imageUrl}" alt=""></div>
						</c:if>
						<div class="actions">
							<span>💬</span><span>🔁 ${pinnedPost.repostsCount}</span><span>❤ ${pinnedPost.likesCount}</span>
						</div>
					</div>
				</div>
			</c:if>
				...
				<div class="post">
					...
					<div class="body">
						<div class="meta">
							...
							<c:catch var="fmtErr"><fmt:formatDate value="${createdRaw}" pattern="yyyy-MM-dd HH:mm" var="createdFmt"/></c:catch>
							<c:choose>
								<c:when test="${empty fmtErr and not empty createdFmt}">${createdFmt}</c:when>
								<c:otherwise>
									<c:catch var="parseErr"><fmt:parseDate value="${createdRaw}" pattern="yyyy-MM-dd HH:mm:ss" var="createdParsed"/></c:catch>
									<c:choose>
										<c:when test="${empty parseErr and not empty createdParsed}">
											<fmt:formatDate value="${createdParsed}" pattern="yyyy-MM-dd HH:mm"/>
										</c:when>
										<c:otherwise><c:out value="${createdRaw}"/></c:otherwise>
									</c:choose>
								</c:otherwise>
							</c:choose>
						</div>
						...
						<c:if test="${not empty post.imageUrl}">
							<div class="photo"><img src="${ctx}/upload/${post.imageUrl}" alt=""></div>
						</c:if>
						<div class="actions">
	...
	<aside class="rightbar">
		<section class="widget">
			<h4>팔로우 추천</h4>
			<div class="follow-list" style="display:flex;flex-direction:column;gap:12px;">
				<c:forEach var="u" items="${recommendedUsers}">
					<!-- 추천 유저도 nickname 우선, 없으면 userId -->
					<c:set var="rName" value="${not empty u.nickname ? u.nickname : (not empty u.userId ? u.userId : 'User')}" />
					<div style="display:flex;align-items:center;justify-content:space-between;">
						<div style="display:flex;align-items:center;gap:10px;min-width:0;">
							...
							<div>
								<div style="font-weight:700;color:var(--color-font);line-height:1.2;">
									<c:out value="${rName}"/>
								</div>
