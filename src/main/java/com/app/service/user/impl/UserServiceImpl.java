...
public class UserServiceImpl implements UserService {
	...
	public User checkUserLogin(User user) {
		 ...
		 * 
		 * //사용자 정보를 조회해서, id pw 맞나 확인 User loginUser =
		 * userDAO.findUserById(user.getId());
		 * 
		 * // if(loginUser != null) if( loginUser != null &&
		 * loginUser.getPw().equals(user.getPw()) &&
		 * loginUser.getUserType().equals(user.getUserType()) ) { return loginUser; } //
		 * checkUserLogin 메소드 호출 -> return null? id,pw 틀렸다 // return user객체 ? -> 맞다!
		 * 
		 * return null;
		 */
